if (typeof AUTO_TITLE != 'undefined' && AUTO_TITLE == true) {
  ...
  function getS3Data(marker, html) {
    ...
    // set loading notice
    $('#listing')
        .html('<img src="//' + BUCKET_NAME + '/ajaxload-circle.gif" />');
    $.get(s3_rest_url)
        .done(function(data) {
  ...
  function buildNavigation(info) {
    ...
    if (info.prefix) {
      ...
      var content = $.map(info.prefix.split('/'), function(pathSegment) {
        ...
        return '<a href="?prefix=' + processedPathSegments + '">' + encodeURI(pathSegment) +
               '</a>';
      });
  ...
  function createS3QueryUrl(marker) {
    ...
    if (match) {
      var uriObj = parseURI(match[1]);
      prefix = S3B_ROOT_DIR + uriObj.pathname;
    } else {
      if (S3BL_IGNORE_PATH) {
    ...
    if (prefix) {
      // make sure we end in / and replace double slashes
      var prefix = prefix.replace(/\/$/, '').replace(/\/\//g, '/') + '/';
